...
using UnityEngine;
using static HomaGames.GDPR.Settings;
...
namespace HomaGames.GDPR
{
    public static class Localization
    {
        ...
        public static SupportedLanguages CurrentLanguage
        {
            ...
            set
            {
                currentLanguage = FETCH_LITERALS_FROM_NTESTING ? SupportedLanguages.EN : value;
        ...
        public static void LoadLanguageFromSettings()
        {
            ...
            if (settings != null)
            {
                if (settings.UseDeviceLanguageWhenPossible)
                {
                    ...
                    SupportedLanguages parsedLanguage = SupportedLanguages.EN;
                    Enum.TryParse(languageIsoCode, true, out parsedLanguage);
                    CurrentLanguage = parsedLanguage;
                    Debug.Log($"[Homa Games GDPR] Loading device language localization file: {CurrentLanguage}");
                }
                else
                {
                    // Use defined language in settings
                    CurrentLanguage = settings.Language;
                    Debug.Log($"[Homa Games GDPR] Loading settings localization file: {CurrentLanguage}");
                }
            }
            else
            {
                Debug.Log($"[Homa Games GDPR] Could not load settings. Loading default language");
        ...
        public static string GetLocalizedValueForKey(string localizationKey, string defaultValue = "")
        {
            ...
            if (FETCH_LITERALS_FROM_NTESTING)
            {
                try
                {
                    localizedString = GDPRGeryonUtils.GetGeryonDynamicVariableValue<string>(localizationKey.ToUpperInvariant(), defaultValue);
                }
                catch (Exception e)
                {
                    Debug.LogWarning($"[Homa Games GDPR] NTesting value not present for {localizationKey.ToUpperInvariant()} -- {e.Message}");
                }
            }
            ...
            {
                Debug.LogWarning(
                $"[Homa Games GDPR] Trying to load localization value for key {localizationKey}, but dictionary is not loaded or key is not present");
        ...
        private static void LoadLocalizationFile(SupportedLanguages language)
        {
            TextAsset localizationTextAsset =
                (TextAsset) Resources.Load($"homagames.gdpr_{language.ToString().ToLower()}", typeof(TextAsset));
            if (localizationTextAsset != null)
            {
                Dictionary<string, object> localizationDictionary =
                    HomaGames.GDPR.Json.Deserialize(localizationTextAsset.text) as Dictionary<string, object>;
            ...
            {
                Debug.LogWarning($"[Homa Games GDPR] Langugage file for {language} not found");
            }
        }
        ...
        public static string Get2LetterISOCodeFromSystemLanguage(SystemLanguage lang)
        {
            ...
            if (!Enum.IsDefined(typeof(SupportedLanguages), res))
            {
                res = "EN";
            }
