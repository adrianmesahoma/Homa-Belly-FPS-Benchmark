...
namespace HomaGames.HomaBelly
{
    ...
    public class HomaBellyUpdateChecker
    {
        ...
        private static PluginManifest latestFetchedPluginManifest;
        ...
        static void CheckUpdateUponLoad()
        {
            ...
            UnityEngine.Debug.Log("Checking for manifest update...");
            ...
            if (oldPluginManifest != null)
            {
                FetchPluginManifest(oldPluginManifest.AppToken).ContinueWith((manifestResult) =>
                {
                    ...
                    if (newPluginManifest != null)
                    {
                        ...
                        latestFetchedPluginManifest = newPluginManifest;
                        updateAvailable = !oldPluginManifest.Equals(newPluginManifest);

                        if (updateAvailable)
                        {
                            UnityEngine.Debug.Log("Homa Belly manifest update available");
                        ...
                        {
                            UnityEngine.Debug.Log("Homa Belly manifes up to date");
                        }
        ...
        {
            return new EditorHttpCaller<PluginManifest>()
                .Get(string.Format(HomaBellyConstants.API_APP_BASE_URL, token), new PluginManifestDeserializer(false));
        }
        ...
        private static void TryShowUpdatePopup()
        {
            if (ShouldShowUpdatePopup())
            {
                ...
                Task.Delay(10000).ContinueWith((result) =>
                {
                    ...
                    if (updateHomaBelly)
                    {
                        new PluginController().InstallPluginManifest(latestFetchedPluginManifest);
                    }
                    else
                    {
                        UnityEngine.Debug.Log($"Homa Belly update will be reminded within next {HOURS_UNTIL_NEXT_UPDATE_POPUP} hours");
                    }
                }, TaskScheduler.FromCurrentSynchronizationContext());
            }
        }
        ...
        private static bool ShouldShowUpdatePopup()
        {
            ...
            if (EditorPrefs.HasKey(LAST_MANIFEST_AVAILABLE_STRING)
                && EditorPrefs.GetString(LAST_MANIFEST_AVAILABLE_STRING) != latestFetchedPluginManifest.ToString())
            {
                return true;
            }
            ...
            {
                long lastUpdatePopupShowFileTime;
                long.TryParse(EditorPrefs.GetString(LAST_UPDATE_POPUP_SHOW), out lastUpdatePopupShowFileTime);
        ...
        private static bool ShouldCheckForUpdate()
        {
            ...
            if (EditorPrefs.HasKey(LAST_UPDATE_CHECK))
            {
                ...
                long.TryParse(EditorPrefs.GetString(LAST_UPDATE_CHECK), out lastUpdateCheckFileTime);
            ...
            return PluginManifest.LoadFromLocalFile() == null ? false : true;
        }
